...
#########################################################################
#
# oci_curl.sh: Used to send REST API request to OCI by curl.
#
# Chenlu Chen - 09/29/2018 Add proxy support
# Chenlu Chen - 09/25/2018 created
#
#################################################################################################################

# For test usage
#export {http,https,ftp}_proxy=www-proxy.us.oracle.com:80

# Get proxy from config file.
#CUR_DIR=$(echo "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)")
#oci_proxy=$(cat ${CUR_DIR}/../config/config-oci.json | jq -r '.oci_proxy')
#export {http,https,ftp}_proxy=${oci_proxy}
...
function oci-curl {
    ...
    curl --tlsv1.2 "${extra_args[@]}" "${body_arg[@]}" -X $curl_method -sS https://${host}${escaped_target} "${curl_header_args[@]}" \
...
######################## Main ################################
# Usage:
# oci_curl.sh <key-id> <private-key-path> <host> <method> [file-to-send-as-body] <request-target> [extra-curl-args]
#
# ex:
# oci_curl.sh <$tenancyId/$authUserId/$keyFingerprint> /root/.oci/oci_api_key.pem iaas.us-ashburn-1.oraclecloud.com get "/20160918/instances?compartmentId=some-compartment-ocid"
# oci_curl.sh <$tenancyId/$authUserId/$keyFingerprint> /root/.oci/oci_api_key.pem iaas.us-ashburn-1.oraclecloud.com post ./request.json "/20160918/vcns"
