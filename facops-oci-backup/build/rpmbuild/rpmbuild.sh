...
function download_artifacts() {
    ...
    curl -o $RPMBUILD_DIR/artifacts/opc_linux64.zip -O https://artifactory-master.cdaas.oraclecloud.com/artifactory/list/generic-fa/opc_linux64.zip | tee -a $LOGFILE
    # 
...
function rpm_deploy() {
    ...
    find $RPM_REPO_DIR -type f -exec curl -s -H 'X-JFrog-Art-Api:AKCp8jQwqTVLHXuJ3wqGNMrYLBPpRvZX5FtDdNQjHdGXze1iX8Rcxm6vTUV9WthL7EZwx3W3F' -X PUT ${REPO_URL}/{} -T {} \; | tee -a $LOGFILE
    if [[ $? -ne 0 ]];then
        "failed to update artifactory rpm repo ..." | tee -a $LOGFILE
        exit 1
    fi
