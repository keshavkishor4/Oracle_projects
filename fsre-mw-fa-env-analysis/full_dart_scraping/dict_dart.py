...
def get_pod_dart_data(pod_url: dict = None,podname: str=None,start_time: str=None,end_time: str=None)-> list:
    ...
    try:
        if not pod_url:
            ...
            with ThreadPoolExecutor() as executer:
                ...
                for i in result:
                    print(i,"\n\n\n")
                    dart_pod.extend(i[0]),all_db_art.extend(i[1]),all_ess_art.extend(i[2]),all_bi_art.extend(i[3]),all_mt_art.extend(i[4])
            ...
            dart_pod, all_db_art, all_ess_art, all_bi_art, all_mt_art=dart_multi_module_data(pod_url=pod_url)
...
def dart_multi_module_data(podname: str=None,dd: str=None,sd_time_obj: str=None,ed_time_obj: str=None,pod_url: dict = None)-> dict:
    ...
    try:
        ...
        if not pod_url:
            ...
            url_array=pod_url["base_url"]
            time_array=pod_url["time_array"]
            color_array=pod_url["color_array"]
        ...
        for value in bi_dart_array:
            if value["url"] :
                if value["dict_data"]:
                    dart_pod.append(value["dict_data"])
                    if "db_performance" in value["dict_data"]:
                        if value["dict_data"]["db_performance"]:
                            if value["db_dart_dict"]:
                                ...
                                all_db_art.append(value["db_dart_dict"])
                    if "essart_summary" in value["dict_data"]:
                        if value["dict_data"]["essart_summary"]:
                            if value["ess_dart_dict"]:
                                ...
                                all_ess_art.append(value["ess_dart_dict"])
                    if "biart_summary" in value["dict_data"]:
                        if value["dict_data"]["biart_summary"]:
                            if value["bi_dart_dict"]:
                                ...
                                all_bi_art.append(value["bi_dart_dict"])
                    if "middle_tier_servers_summary" in value["dict_data"]:
                        if value["dict_data"]["middle_tier_servers_summary"]:
                            if value["mt_dart_dict"]:
                                ...
                                all_mt_art.append(value["mt_dart_dict"])
                else:
...
test={
  "pod_name":"HDSS",
  "base_url":["https://dashboards.odin.oraclecloud.com/DART/HDSS_20231217235951396226_DARTAutomatedAnalysis/ART_Summary.html","https://dashboards.odin.oraclecloud.com/DART/HDSS_20231217232525209370_DARTAutomatedAnalysis/ART_Summary.html","https://dashboards.odin.oraclecloud.com/DART/HDSS_20231217225651637453_DARTAutomatedAnalysis/ART_Summary.html"],
  "time_array":["2023-12-17_23","2023-12-17_23","2023-12-17_22"],
  "color_array":["","",""]
  }

def main():
    #get_pod_dart_data(pdoname,starttime,endtime)
    dart_pod, all_db_art, all_ess_art, all_bi_art, all_mt_art=get_pod_dart_data(pod_url=test)
    json_file_dump(all_db_art,"test.json","","pod")
