...
variable "faops_sopsvm_ad_indices" {
  type = "list"
  default = [0]
}
...
variable "faops_sopsvm_8203_health_checker_url" {
    default = "/"
}
#This would need a change when the final health check url is finalized
variable "faops_sopsvm_8080_health_checker_url" {
    default = "/"
}

variable "faops_sopsvm_local_volume_size_in_gbs"  {
    default = ""
}

variable "faops_sopsvm_region" {
  default=""
}
...
locals {
  ...
  region_check = "${lookup(local.faops_sopsvm_region_map,var.realm,"us-ashburn-1") == var.region || lookup(local.faops_sopsvm_dr_region_map,var.realm,"us-ashburn-1") == var.region ? 0 : 1}"
}
