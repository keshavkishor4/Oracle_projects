...
variable "faops_provutilvm_region" {
  default = "us-ashburn-1"
}
...
locals {
  ...
  faops_provutilvm_ad_indices_map = {
    prd = [0]
    ppd = [0]
    dev = [0]
    tst = [0]
  ...
  # DR
  faops_provutilvm_dr_region_map = {
    prd = "us-phoenix-1"
    ppd = "${var.region}"
    dev = "${var.region}"
    tst = "${var.region}"
...
variable "faops_provutilvm_lb_shape" {
  default = "100Mbps"
}
...
locals {
  ...
  region_check = "${lookup(local.faops_provutilvm_region_map,var.realm,"us-ashburn-1") == var.region || lookup(local.faops_provutilvm_dr_region_map,var.realm,"us-phoenix-1") == var.region ? 0 : 1}"
}
