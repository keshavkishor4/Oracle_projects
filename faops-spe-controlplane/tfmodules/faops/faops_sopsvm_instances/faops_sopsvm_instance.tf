...
module "faops_sopsvm" {
  ...
  instance_display_name = "${var.server}_sopsvm"
  instance_hostname_label = "${var.server}sopsvm"
  ad_list = "${var.ad_list}"
  subnet_ids = "${var.faops_mt_subnet_ids}"
  ad_indices = "${var.faops_sopsvm_ad_indices}"
  volume_mount_directory = "/u01"
  volume_size_in_gbs = "${var.faops_sopsvm_local_volume_size_in_gbs}"
...
output "faops_sopsvm" {
  value = ["${formatlist("%s: %s (%s)", module.faops_sopsvm.display_name, module.faops_sopsvm.private_ip, module.faops_sopsvm.availability_domain)}",]
}

output "faops_sopsvm_private_ips" {
  value = "${module.faops_sopsvm.private_ip}"
}
