...
module "faops_provutilvm" {
  source = "../../../tfmodules/linux_instance_faops/psr"
  compartment_id= "${var.app_compartment_id}"
  instance_display_name = "${var.server}_provutilvm"
  instance_hostname_label = "${var.server}provutilvm"
  ad_list = "${var.ad_list}"
  subnet_ids = "${var.faops_mt_subnet_ids}"
  ad_indices = "${var.faops_provutilvm_ad_indices}"
  volume_mount_directory = "/u01"
  volume_size_in_gbs = "${var.faops_provutilvm_local_volume_size_in_gbs}"
...
output "faops_provutilvm" {
  value = ["${formatlist("%s: %s (%s)", module.faops_provutilvm.display_name, module.faops_provutilvm.private_ip, module.faops_provutilvm.availability_domain)}",]
}

output "faops_provutilvm_private_ips" {
  value = "${module.faops_provutilvm.private_ip}"
}
