...
module "faops_perftools" {
  source = "../../../tfmodules/linux_instance_faops/psr"
  compartment_id= "${var.app_compartment_id}"
  instance_display_name = "${var.server}_perftools"
  instance_hostname_label = "${var.server}perftools"
  ad_list = "${var.ad_list}"
  subnet_ids = "${var.faops_mt_subnet_ids}"
  ad_indices = "${var.faops_perftools_ad_indices}"
  volume_mount_directory = "/u01"
  volume_size_in_gbs = "${var.faops_perftools_local_volume_size_in_gbs}"
...
output "faops_perftools" {
  value = ["${formatlist("%s: %s (%s)", module.faops_perftools.display_name, module.faops_perftools.private_ip, module.faops_perftools.availability_domain)}",]
}

output "faops_perftools_private_ips" {
  value = "${module.faops_perftools.private_ip}"
}
