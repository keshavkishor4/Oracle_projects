...
def get_podvms_format(pod,account):
    result = podcontent.pod_instance_data_values(pod,account)
    PODVMs=[]
    for i in range(len(result['podHosts'])):
        if str(result['podHosts'][i]['systemType']) == "ADMIN-APPS":
            PODVMs.append("FA")  
        elif "IDMFA" in str(result['podHosts'][i]['systemType']):
            tmp=re.sub('IDMFA', 'IDM',str(result['podHosts'][i]['systemType']))
            PODVMs.append(tmp) 
        elif "OHSFA" in str(result['podHosts'][i]['systemType']):      
            tmp=re.sub('OHSFA', 'OHS',str(result['podHosts'][i]['systemType']))
            PODVMs.append(tmp)               
        else:    
            PODVMs.append(str(result['podHosts'][i]['systemType']))
    PODVMtmp=[re.sub('-', '',i) for i in PODVMs]   
    PODVMs=[]
    for i in PODVMtmp:
        if "OPT" in i:
            PODVMs.append(i)
        else:
            PODVMs.append(re.sub('[0-9]', '',i))    
    return PODVMs            
   
def get_opt_vms(stamp_size):
    rename = { "GRC": "OPT1", "VCP": "OPT2", "ES": "OPT3", "BIP": "OPT4"}
    for i, j in rename.items():
        opt_vms = stamp_size.replace(i, j)
    opt_vms_exist=re.findall("(\w+)=1,", opt_vms)
    return opt_vms_exist

####################################################################PODSIZE COMP####################################################################
def current_size_temp(pod,account,profile_data,release,arc,pod_size):
    PODVMs=get_podvms_format(pod,account)
    #print(f'POD VMS -> {PODVMs}')
    try:
        size=""
        cur_scaling_no = ""
        ...
        for profile in profile_data:
            if profile["podSizeType"] == size and profile["faRelease"] == release and profile["profileType"] == "INFRASTRUCTURE":
                #print("# ------ PROFILE ------ #")
                #print(f'size -> {pod_size} :: release -> {release}')
                #print(profile)
                #print("# ------ PROFILE ---- END -- #")
                for vmtype in json.loads(profile["sizeProfileJsonData"])["topology"]["vmHosts"]:
                    #print(f'pod VM profile -> {vmtype["vmType"]}')
                    if vmtype["vmType"] in PODVMs:    
                        if "ocpus" in vmtype["ociVMShapeHwInfo"][arc]:
                            ...
                            shape=vmtype["ociVMShapeHwInfo"][arc]["shape"]
                if "AUXVMSCALE" in PODVMs:
                    for auxvm in json.loads(profile["sizeProfileJsonData"])["topology"]["extensionDefinitions"]:
                        if "ocpus" in auxvm ["ociVMShapeHwInfo"][arc]:
                                vmdic.update({auxvm["vmType"]:{"arc":arc,"shape":auxvm["ociVMShapeHwInfo"][arc]["shape"],"ocpus":auxvm["ociVMShapeHwInfo"][arc]["ocpus"],"memoryInGBs":auxvm["ociVMShapeHwInfo"][arc]["memoryInGBs"]}})
                        else:
...
def target_size_temp(pod,account,profile_data,release,tar_arc,target_size):      
    PODVMs=get_podvms_format(pod,account)
    #print(f'POD VMS -> {PODVMs}')
    try:
        trg_scaling_no=""
                ...
                if "AUXVMSCALE" in PODVMs:            
                    for auxvm in json.loads(profile["sizeProfileJsonData"])["topology"]["extensionDefinitions"]:
                        if "ocpus" in auxvm ["ociVMShapeHwInfo"][tar_arc]:
                                vmdic.update({auxvm["vmType"]:{"arc":tar_arc,"shape":auxvm["ociVMShapeHwInfo"][tar_arc]["shape"],"ocpus":auxvm["ociVMShapeHwInfo"][tar_arc]["ocpus"],"memoryInGBs":auxvm["ociVMShapeHwInfo"][tar_arc]["memoryInGBs"]}})
                        else:
...
def pod_sizing_comp(pod,target_size,src_arc,tar_arc,account,release=None,pod_size=None):
    try:
        ...
        if not release:
            print("{0}error occured in getting FA release".format(globalvariables.RED))
        status,file_name=datafetch.resize_lcm_profile_data(account)
        #print(f'%%%%%% DOPE filename :: {file_name}')
        if status:
            with open(file_name,'r') as j_data:
                response=json.load(j_data)
            current_temp=current_size_temp(pod,account,response,release,src_arc,pod_size)
            #print(f'^^^^ 11 current_temp => {current_temp}')
            targate_temp=target_size_temp(pod,account,response,release,tar_arc,target_size)
...
def extract_size_info(size):
    match = re.match(r"([A-Za-z]+)(\d+)", size)
    if match:
        size_code, size_number = match.groups()
        return size_code, int(size_number)
    return None, None

def compare_sizes(target_size, source_size):
    size_order = {"XS": 1, "S": 2, "M": 3, "L": 4}
    target_size_code, target_size_number = extract_size_info(target_size)
    source_size_code, source_size_number = extract_size_info(source_size)

    target_order = size_order.get(target_size_code, 0)
    source_order = size_order.get(source_size_code, 0)
    if target_order > source_order:
        return "T-Shirt"
    elif target_order == source_order:
        if int(target_size_number) > int(source_size_number):
            return "ScaleOut"
        elif int(target_size_number) == int(source_size_number):
            return "No Change"
        else:
            return "ScaleDown"
    elif target_order < source_order:
                    ...
                    for i in jdata:
                        ...
                        if hw_map:
                            ...
                            if ( region and  i["FM Date"] == input_date and i["Data Center"] == region ) or ( not region and  i["FM Date"] == input_date ):
                                if base_size in size_template and release in release_list:
    ...
    try:
        ...
        for pod_data in dict1:
            for pod in pod_data.values():
                ...
                if vmshape not in globalvariables.x_shape:
                    for vm,value in pod["Old Shape"]["vmtype"].items():
                        if vm != "AUXVM" and "ocpus" in list(value):
                            ...
                            if status:#checking if shape already exist in json
                                ...
                                vms_temp=old_dict1[vmshape][ind]["vm"]
                                vms_temp.append(vm)
                                old_dict1[vmshape][ind].update({"ocpus":av_shape["ocpus"],"memoryInGBs":av_shape["memoryInGBs"],"count":newcount,"vm":vms_temp})
                            else:
                                old_dict1[vmshape].append({"ocpus":value["ocpus"],"memoryInGBs":value["memoryInGBs"],"count":1,"vm":[vm]})
                        if vm == "AUXVM" and auxvm >0:
                                old_dict1[vmshape].append({"ocpus":value["ocpus"],"memoryInGBs":value["memoryInGBs"],"count":auxvm,"vm":[vm]})
        ...
        for pod_data in dict1:
            for pod_req in pod_data.values():
                ...
                #AMD type processing
                if vmshape not in globalvariables.x_shape:
                    for vm,value in pod["vmtype"].items():
                        if vm != "AUXVM" and "ocpus" in list(value):
                            #print(f'NAX vm:value -> {vm}:{value}')
                            ...
                            if not status_old:
                                ...
                                if status_new:
                                    ...
                                    vms_temp=new_dict[vmshape][ind]["vm"]
                                    vms_temp.append(vm)
                                    new_dict[vmshape][ind].update({"ocpus":shape_update["ocpus"],"memoryInGBs":shape_update["memoryInGBs"],"count":newcount,"vm":vms_temp})
                                else:
                                    new_dict[vmshape].append({"ocpus":value["ocpus"],"memoryInGBs":value["memoryInGBs"],"count":1,"vm":[vm]})
                                # print(f'NEW DICT NAX-> {new_dict}')        
                        ...
                        # print(f'NEW DICT NAX:end-> {new_dict}')        
                        #AUXVM count processing
                        if vm == "AUXVM" and auxcount >0:
                            #print(f'AUX key:value -> {vm}:{value}')
                ...
                #intel type processing
                    ...
                    for intel_key,intel_value in pod["vmtype"].items():
                        if intel_key != "AUXVM" and "shape" in list(intel_value):
                        ...
                        if intel_key == "AUXVM" and auxcount >0:#AUXVM processing
                            ...
                            elif old_dict[intel_value["shape"]] > auxcount:
                                    newcount = old_dict[intel_value["shape"]] - auxcount
                                    old_dict[intel_value["shape"]] = newcount
                            else:
                                newcount=auxcount - old_dict[intel_value["shape"]]
                                new_dict[intel_value["shape"]] =  newcount
                                old_dict.pop(intel_value["shape"])
        old_dict1=check_dict_format(old_dict1)
        new_dict=check_dict_format(new_dict)
        #print(f'Old_dict -> {old_dict1}')
...
def pod_capacity_analysis(pod,target_size,src_arc,tar_arc,account):
    ...
    pod_list.append({pod:{"Old Shape":current,
                       "New Shape":target}})
    if len(pod_list) != 0:
        #print(" ****** Function: capacity_calculate_v1: START")
        current_uses,capacity_require=capacity_calculate_v1(pod_list)
        print(f'\n#Cap_uses -> {current_uses}') 
        print(f'#Cap_Need -> {capacity_require}\n')
        #print(" ****** Function: capacity_calculate_v1: END")
        ...
        #print("%%% pod_capacity_analysis %%")
        #print(pod_dict)
    return current, pod_dict

# region_manual_psr_data("12-MAY-23","commercial","ap-sydney-1")

# data = podcontent.full_pod_data_method("EEHO","commercial")
# print(data)

# pod_sizing_comp("EEHO","L70","X6","X6","commercial")


#podcapacity uses matrics captured on apex
def run_query(query):
    try:
...
def session_json():
    try:
        ...
        user_name=globalvariables.env_dictionary["commercial"]["cloud_meta"]["username"]
        json_file=globalvariables.session_json
        response=run_query(query)
        if os.path.exists(json_file):
            with open(json_file,'r') as j_d:
                j_file_data=json.load(j_d)
        for session in response:
            ses_key_persent=False
            session_dict={}
            session_dict.update({"user_name":user_name,"session_key":session[0],"session_data":session[1],"expire_date":session[2]})
            if j_file_data:
                for ses in j_file_data:
                    if ses["session_key"] == session[0] and ses["session_data"] == session[1]:
                        ses_key_persent=True
            if not ses_key_persent:
                j_file_data.append(session_dict)
                post_session_data(session_dict)
        #writing session json to file.
        with open(json_file, 'w') as outfile:
            json.dump(j_file_data, outfile, indent=4, sort_keys=False)
    except Exception as e:
        traceback.print_exc()
        print("{0}error in executing db {1}{2}".format(globalvariables.RED,e,globalvariables.RESET))

#postintg session data to apex
def post_session_data(session_json):
    try:
        url=globalvariables.post_session_url
        #print("posting :",session_json)
        response = requests.post(url, json = session_json)
        #print("posting session data {0}".format(response))
    except Exception as e:
        traceback.print_exc()
        print("{0}error in posting session data to apex {1}{2}".format(globalvariables.RED,e,globalvariables.RESET))
